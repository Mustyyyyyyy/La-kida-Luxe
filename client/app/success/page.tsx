      ...
      fallback={
        <main className="page">
          <header className="sticky top-0 z-40 topbar px-6 lg:px-20 py-4 flex items-center justify-between">
            <BrandLogo size={54} />
            <div className="btn-outline px-5 py-2 text-xs">Loading...</div>
          </header>

          <section className="px-6 lg:px-20 py-16">
            <div className="max-w-[900px] mx-auto card p-8 md:p-10 text-center">
              <p className="muted">Preparing your order confirmation...</p>
            </div>
          </section>
        </main>
      }
  ...
  const waMessage = useMemo(() => {
    ...
    const lines: string[] = [
      ...
      ...items.map((it: any) => {
        ...
        return `- ${it.title} x${it.qty}${size}${color}`;
      }),
      "",
      `Subtotal: ${subtotal}`,
      ...
      bankName || accountName || accountNumber
        ...
        : `Please send payment details so I can complete payment.`,
      bankName ? `Bank: ${bankName}` : "",
      accountName ? `Account Name: ${accountName}` : "",
      accountNumber ? `Account Number: ${accountNumber}` : "",
      "",
      `After payment, I’ll send proof of payment here. Thank you 🙏🏽`,
    ].filter(Boolean);

    return lines.join("\n");
  }, [summary, orderCode, orderId, settings]);
      ...
      <section className="px-6 lg:px-20 py-16">
        <div className="max-w-[900px] mx-auto card p-8 md:p-10 text-center">
          ...
          <p className="mt-3 muted">
            Next step: tap WhatsApp to confirm your order and get payment instructions.
          </p>
          ...
          <p className="mt-6 text-xs muted">
            Your order stays as <b>Pending</b> until you chat and complete payment on WhatsApp.
          </p>
        </div>
      </section>
    </main>
