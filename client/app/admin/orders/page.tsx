...
export default function AdminOrdersPage() {
  ...
  async function onDelete(id: string) {
    ...
    try {
      setBusyId(id);
      await apiFetchAuth(`/api/orders/${id}`, { method: "DELETE" });
    ...
    <div className="space-y-8">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm muted">Manage WhatsApp orders and delivery status.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1150px] w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="py-10 px-5 muted">
                    Loading orders...
                  </td>
                </tr>
              ) : items.length === 0 ? (
                ...
                items.map((o) => (
                  <tr key={o._id} className="border-b border-white/10">
                    ...
                    <td className="py-3 px-5">
                      <div className="text-xs muted">{o.delivery?.method || "standard"}</div>
                      <div className="text-sm text-white/90">{o.delivery?.address || "—"}</div>
                    </td>
